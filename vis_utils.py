...
def format_attention(attention, layers=None, heads=None):
    ...
    for layer_attention in attention:
        # Expected shape: 1 x num_heads x seq_len x seq_len
        if len(layer_attention.shape) != 4:
            print(layer_attention.shape)
            raise ValueError(
                "The attention tensor does not have the correct number of dimensions. "
                "Make sure you set output_attentions=True when initializing your model."
            )
    ...
    """Render head view
        ...
        Args:
            For self-attention models:
                attention: list of FloatTensor (one per layer) of shape
                    ``(batch_size(must be 1), num_heads, sequence_length, sequence_length)``
                tokens: list of tokens
                sentence_b_start: index of first wordpiece in sentence B (optional)
            For encoder-decoder models:
                encoder_attention, decoder_attention, cross_attention:
                    same shape as above but for each sub-module
                encoder_tokens, decoder_tokens: corresponding token lists
            For all models:
                prettify_tokens: strip wordpiece artefacts (Ġ, ▁, </w>)
                layer: zero-based index of initially selected layer
                heads: zero-based indices of initially selected heads
                include_layers: subset of layers to render
                html_action: 'view' (display inline) or 'return' (return HTML)
    """
    def _to_numpy(t):
